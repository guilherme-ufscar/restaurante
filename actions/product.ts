...
export async function createProduct(data: z.infer<typeof productSchema>, restaurantId: string) {
    try {
        ...
        // Verificar limites do plano e status da assinatura
        if (restaurant.subscriptionStatus !== "ACTIVE") {
            return {
                success: false,
                message: "Sua assinatura não está ativa. Por favor, regularize seu plano para gerenciar produtos."
            }
        }
...
export async function updateProduct(id: string, data: z.infer<typeof productSchema>) {
    try {
        ...
        if (product.restaurant.subscriptionStatus !== "ACTIVE") {
            return {
                success: false,
                message: "Sua assinatura não está ativa. Por favor, regularize seu plano para gerenciar produtos."
            }
